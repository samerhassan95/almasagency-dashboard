...
interface ServiceFormProps {
  initialData?: { id?: number; title_ar: string; title_en: string; description_ar: string; description_en: string; image_url?: string };
  mode: "create" | "edit";
}
...
export default function ServiceForm({ initialData, mode }: ServiceFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    description_ar: initialData?.description_ar || "", description_en: initialData?.description_en || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = mode === "edit" ? `${apiUrl}/services/${initialData?.id}` : `${apiUrl}/services`;
  ...
  return (
    <form onSubmit={handleSubmit} className="admin-form">
      ...
      <div className="admin-form-group">
        ...
        <div className="admin-upload-zone" onClick={() => fileInputRef.current?.click()}>
          {uploading ? <p style={{ color: "#7c3aed" }}>جارٍ الرفع...</p>
            : form.image_url ? <img src={getFullImageUrl(form.image_url)} alt="preview" className="admin-upload-preview" />
      ...
      <div className="admin-section-divider"><span>الاسم</span></div>
      <div className="admin-form-row">
        <div className="admin-form-group">
          <label className="admin-label">اسم الخدمة <span className="admin-label-lang admin-label-lang--ar">عربي</span></label>
          <input name="title_ar" value={form.title_ar} onChange={handleChange} required className="admin-input" dir="rtl" />
        </div>
        <div className="admin-form-group">
          <label className="admin-label">Service Name <span className="admin-label-lang admin-label-lang--en">EN</span></label>
          <input name="title_en" value={form.title_en} onChange={handleChange} required className="admin-input" dir="ltr" />
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
        <Link href="/admin/services" className="admin-btn admin-btn--outline"><X size={16} /> إلغاء</Link>
