      ...
      <div className="admin-card">
        <div className="admin-table-wrap">
          <table className="admin-table">
            ...
            <tbody>
              {projects.length === 0 ? (
                <tr><td colSpan={5}><div className="admin-empty"><p>لا توجد مشاريع بعد</p></div></td></tr>
              ) : (
                projects.map((p: any) => (
                  <tr key={p.id}>
                    <td>
                      {p.image_url
                        ? <img src={getFullImageUrl(p.image_url)} alt={p.title_ar} className="admin-table-img" />
                    ...
                    <td>
                      <div style={{ display: "flex", gap: 8 }}>
                        <Link href={`/admin/projects/${p.id}/edit`} className="admin-btn admin-btn--outline admin-btn--sm">
